...
from Data.CertProvisioningVariable import CertProvisioningApplyVariable     # noqa: E402
from Data.CertProvisioningVariable import CertProvisioningResultVariable    # noqa: E402
...
def TestSignSemTrustedCert(options):
    ...
    return DetachedSignWithSignTool(
        gPath2SignTool,
        options.CertFilePath,
        options.Signing2AOutputFile,
        options.Signing2APfxFile,
        options.Signing2APfxPw,
        gOid,
    )
...
def main():
    ...
    parser.add_argument(
        "-p", dest="PrintFile", help="Print File as Provisioning Blob", default=None
    )
    parser.add_argument(
        "-pr",
        dest="PrintResultsFile",
        help="Print Result File as Identity Blob",
        default=None,
    )
    parser.add_argument(
        "-pc",
    ...
    parser.add_argument(
        "-xc",
        dest="ExtractCertFile",
        help="Extract the certificate to {basename}_{certtype}.cer",
        default=None,
    )
    parser.add_argument(
        "--dirty",
    ...
    Step1Group.add_argument(
        "--Identity",
        dest="Identity",
        help="Identity (Owner=1, User=2, User1=3, User2=4, Ztc=5).  Default is Owner",
        default=1,
    )
    Step1Group.add_argument(
        "--SnTarget",
    ...
    Step1Group.add_argument(
        "--CertFilePath",
        dest="CertFilePath",
        help="Path to binary DER Cert",
        default=None,
    )
    Step1Group.add_argument(
        "--PrepResultFile",
        dest="PrepResultFile",
        help="Optional File for output from Step1. Required if not doing step2 or step2A",
        default=None,
    )
    Step1Group.add_argument(
        "--HdrVersion",
        ...
        default=CertProvisioningApplyVariable.VERSION_V1,
    )
    Step1Group.add_argument(
        "--SMBIOSMfg",
        dest="SMBIOSMfg",
    ...
    Step1Group.add_argument(
        "--Version", dest="Version", help="Specify Identity version", default=0
    )
    Step1Group.add_argument(
        "--Lsv", dest="Lsv", help="Specify the lowest supported version", default=0
    )

    Step2AGroup = parser.add_argument_group(
        title="Step2A", description="Test Signature Generation Step."
    )
    Step2AGroup.add_argument(
        "--Step2AEnable",
        dest="Step2AEnable",
        help="Do Step 2A - Local Signing for Test Signature",
        default=False,
        action="store_true",
    )
    ...
    Step2AGroup.add_argument(
        "--Signing2AResultFile",
        dest="Signing2AResultFile",
        help="Optional File for output from Step2A. Required if not doing step2B",
        default=None,
    )
    Step2AGroup.add_argument(
        "--Signing2APfxFile",
        dest="Signing2APfxFile",
        help="Path to PFX file for signing Test Signature ",
        default=None,
    )
    Step2AGroup.add_argument(
        "--Signing2APfxPw",
        dest="Signing2APfxPw",
        help="Optional Password for PFX file for signing Test Signature",
        default=None,
    )

    Step2BGroup = parser.add_argument_group(
        title="Step2B", description="Package Cert Provision With Test Signature."
    )
    Step2BGroup.add_argument(
        "--Step2BEnable",
        dest="Step2BEnable",
        help="Do Step 2B - Package Test Signature with Cert Provision Data",
        default=False,
        action="store_true",
    )
    Step2BGroup.add_argument(
        "--TestSignatureInputFile",
        dest="TestSignatureInputFile",
        help="Detached Signature file for Test Signature",
        default=None,
    )
    Step2BGroup.add_argument(
        "--CertProvisionBlobInputFile",
        dest="CertProvisionBlobAfterStep1File",
        help="Step1 Output File to use as Input to combine with Test Signature",
        default=None,
    )
    Step2BGroup.add_argument(
        "--Prep2BResultFile",
        dest="Prep2BResultFile",
        help="Optional File for output from Step2B. Required if not doing step2",
        default=None,
    )
    ...
    Step3Group = parser.add_argument_group(
        title="Step3", description="Final Provisioning Var Construction."
    )
    ...
    if options.Step1Enable:
        ...
        if (not options.Step2Enable) and (not options.Step2AEnable):
            ...
            if not options.PrepResultFile:
                logging.critical(
                    "Since Step2A/2 is not enabled there must be a PrepResultFile for the result"
                )
                return -3
    ...
    if options.Step2AEnable:
        ...
        if not options.Signing2APfxFile:
            logging.critical(
                "Since Step2A is enabled you must supply a path to a PFX file for test signing"
            )
            return -848
        ...
        if not options.Step2BEnable:
            ...
            if not options.Signing2AResultFile:
                logging.critical(
                    "Since Step2B is not enabled there must be a Signing2AResultFile for the result"
                )
                return -5

            if options.Signing2AResultFile:
                logging.debug(
                    "Step2A Result will be written to: " + options.Signing2AResultFile
                )
    ...
    if options.Step2BEnable:
        ...
        if not options.Step2AEnable:
            ...
            if (not options.TestSignatureInputFile) or (
                not os.path.isfile(options.TestSignatureInputFile)
            ):
                logging.critical(
                    "Step2B Must have an Test Signature Input File when 2A is not enabled"
                )
                return -8487
        else:
            # Step 2A enabled
            if options.TestSignatureInputFile:
                logging.critical(
                    "Step2B can not have a Test Signature Input File when 2A is enabled"
                )
                return -8489

        if not options.Step1Enable:
            # must have Step1s data file
            if (not options.CertProvisionBlobAfterStep1File) or (
        ...
        if not options.Step2Enable:
            ...
            if not options.Prep2BResultFile:
                logging.critical(
                    "Step2B must have an output file (Prep2BResultFile)  when Step2 is not enabled"
                )
                return -8491
    ...
    if options.Step1Enable:
        ...
        if options.CertFilePath is not None:
