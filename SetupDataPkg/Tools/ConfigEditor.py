...
class application(tkinter.Frame):
    def __init__(self, master=None):
        ...
        file_menu.add_command(
            label="Open Config YAML file...", command=self.load_from_yaml
        )
        file_menu.add_command(
    ...
    def get_open_file_name(self, ftype):
        if self.is_config_data_loaded():
            if ftype == "dlt":
                ...
                question = (
                    "All configuration will be reloaded from BIN file, continue ?"
                )
            elif ftype == "svd":
                question = ""
            elif ftype == "yaml":
    ...
    def get_save_file_name(self, extension):
        path = filedialog.asksaveasfilename(
            initialdir=self.last_dir, title="Save file", defaultextension=extension
        )
        if path:
            self.last_dir = os.path.dirname(path)
            return path
    ...
    def save_delta_file(self, full=False):
        ...
        self.cfg_data_obj.generate_delta_file_from_bin(
            dlt_path, self.org_cfg_data_bin, new_data, full
        )
    ...
    def save_to_svd(self):
        ...
        (execs, bytes_array) = self.cfg_data_obj.generate_delta_svd_from_bin(
            self.org_cfg_data_bin, new_data
        )
    ...
    def save_full_to_svd(self):
        ...
        settings = [("Device.ConfigData.ConfigData", b64data.decode("utf-8"))]
        set_lib = SettingsXMLLib()
        set_lib.create_settings_xml(
            filename=temp_file, version=1, lsv=1, settingslist=settings
        )
            ...
            if itype in ["EditText"]:
                txt_val.trace(
                    ...
                    lambda *args: self.limit_entry_size(
                        txt_val, (self.cfg_data_obj.get_cfg_item_length(item) + 7) // 8
                    ),
                )
